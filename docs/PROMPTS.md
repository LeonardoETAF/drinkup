...
| 7 — Painel admin | 🔄 em andamento | **Parte 1**: casca (sidebar+topo) + dashboard + leads (listar/buscar/filtrar/mudar status). **Parte 2a**: CRUD de produtos (listar/buscar/criar/editar/excluir, slug automático). **Parte 2b**: upload de imagens (handler Axum `POST /upload-imagem`, validação de tipo por magic bytes + tamanho ≤5MB, gravado em `uploads/` e servido via `ServeDir` em `/uploads`; campo de arquivo no form com prévia). RBAC (`exigir_papel`) em toda server fn + middleware 303. Dados do painel carregam **client-side** (Effect+spawn_local) — acessar sessão/banco no SSR com streaming trava (tower-sessions). **Pendente**: eventos, parceiros, usuários/config. |

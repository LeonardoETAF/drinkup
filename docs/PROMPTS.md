...
| 7 — Painel admin | ✅ concluída | **Parte 1**: casca (sidebar+topo) + dashboard + leads (listar/buscar/filtrar/mudar status). **Parte 2a**: CRUD de produtos (listar/buscar/criar/editar/excluir, slug automático). **Parte 2b**: upload de imagens (handler Axum `POST /upload-imagem`, validação de tipo por magic bytes + tamanho ≤5MB, gravado em `uploads/` e servido via `ServeDir` em `/uploads`; campo de arquivo no form com prévia). **Parte 2c**: CRUD de parceiros (grade de cards, upload de logo). **Parte 2d**: "Eventos" = categorias do carrossel da home (nome+cor+ordem+visível; migration 0009 adiciona cor/ordem ao `eventos`). **Parte 2e**: usuários do painel (CRUD **somente admin**: papel, ativo, senha Argon2, sem auto-exclusão) acessível por Configurações; **Configurações** da loja (chave/valor; migration 0010) — nome/CNPJ/contato/horários, com a página pública `/contato` lendo esses dados via fn pública `obter_contato` (SSR). RBAC (`exigir_papel`) em toda server fn + middleware 303. Dados do painel carregam **client-side** (Effect+spawn_local); o item de menu "Usuários" não existe no mockup, então fica acessível dentro de Configurações. |
| 8 — Endurecimento de segurança | ✅ concluída | **CSP baseada em nonce** (release-only, via `ResponseOptions` no render — o nonce casa com os `<script>` da hidratação; em dev é no-op p/ não quebrar o live-reload). **HSTS** (release). Headers sempre: `X-Content-Type-Options`, `Referrer-Policy`, `X-Frame-Options: DENY`, `Permissions-Policy`, `COOP`. **CSRF**: cookie `SameSite=Lax` + middleware `verifica_origem` (403 p/ `Origin` forjada em POST `/api/*` e `/upload-imagem`). **Auditoria do WASM**: sem segredos/SQL/argon2 no bundle (só paths das server fns). Server fns: admin sob `exigir_papel`, públicas intencionais, orçamento com honeypot+throttle. Validado em build de **release** real. Obs.: rate-limit de login é por e-mail (em memória); por IP fica a cargo do proxy reverso na Fase 10. |
| 9 — SEO + desempenho | ✅ concluída | Componente `<Seo>` (título, description, canonical, Open Graph + Twitter) por página (home, catálogo, produto, contato, em-breve). Página de produto em **`SsrMode::Async`** p/ título/OG dependentes de dados caírem no `<head>` do SSR (validado). `robots.txt` (Disallow /admin + Sitemap) e **`/sitemap.xml`** dinâmico (páginas fixas + produtos ativos). **404 real** (status HTTP + título). Imagens: `loading=lazy`+`decoding=async`+dimensões (CLS já coberto por `aspect-ratio`). `Cache-Control: immutable` em `/uploads`. URL base em `components::seo::SITE_URL` = `https://drinkup.com.br`. Pendente futuro: `srcset` (exige gerar múltiplos tamanhos no upload). |
| 10 — Deploy (VPS/Docker) | ✅ concluída | **Dockerfile multi-stage** (builder com cargo-leptos 0.3.6 + wasm, build offline via `.sqlx`; runtime `debian-slim` non-root + healthcheck). **`docker-compose.prod.yml`** (app + Postgres, volumes p/ dados e uploads, healthcheck, app só no loopback atrás de proxy, segredos via env). `.dockerignore` (exclui `/target`, mantém `.sqlx`/`migrations`). `.env.example` com seção de produção. **`docs/DEPLOY.md`**: passo a passo, criação do 1º admin, **proxy reverso Nginx (TLS + rate-limit por IP no `/api/login`)**, backups e checklist. Migrations rodam no startup. |
